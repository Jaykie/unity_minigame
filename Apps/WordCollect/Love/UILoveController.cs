...
public class UILoveController : UIView, ITableViewDataSource
{
    ...
    void UpdateList()
    {
        List<WordItemInfo> ls = LoveDB.main.GetAllItem();
        listItem.Clear();
    ...
    public void OnCellItemDidClick(UICellItemBase item)
    {
        ...
        string strPrefab = "App/Prefab/Game/UIWordDetail";
        if (Common.appKeyName == GameRes.GAME_IDIOM)
        {
            strPrefab = "App/Prefab/Game/UIIdiomDetail";
            PopUpManager.main.Show<UIIdiomDetail>(strPrefab, popup =>
     {
     ...
     }, popup =>
     {
        ...
        {
            PopUpManager.main.Show<UIWordDetail>(strPrefab, popup =>
                 {
                     Debug.Log("UIWordDetail Open ");
                     popup.UpdateItem(info);
