...
public class UIWordImageText : UIWordContentBase
{
    ...
    public void UpdateItem()
    {
        ...
        if (info.gameType == GameRes.GAME_TYPE_TEXT)
        {
            ...
            if ((!Common.BlankString(info.head)) && (!Common.BlankString(info.end)))
            {
                ...
                listAnswerInfo = new List<AnswerInfo>();
                // for (int i = 0; i < info.end.Length; i++)
                {
                    // int idx = 0;
                    //string word_answer = info.end.Substring(idx, 1);
                    AnswerInfo infoanswer = new AnswerInfo();
                    infoanswer.word = info.end;
                    infoanswer.index = 0;
                    infoanswer.isFinish = false;
                    infoanswer.isFillWord = false;
                    listAnswerInfo.Add(infoanswer);
        ...
        else
        {
            ...
            {
                AnswerInfo infoanswer = new AnswerInfo();
                infoanswer.word = GameAnswer.main.GetGuankaAnswer(info, false, 0); ;
                infoanswer.index = 0;
                infoanswer.isFinish = false;
                infoanswer.isFillWord = false;
                listAnswerInfo.Add(infoanswer);
    ...
    {
        Debug.Log("OnRightAnswer idx=" + idx + " listAnswerInfo.Count=" + listAnswerInfo.Count);
        for (int i = 0; i < listAnswerInfo.Count; i++)
        {
            AnswerInfo info = listAnswerInfo[i];
            if (i == idx)
            {
                info.isFinish = true;
                Debug.Log("OnRightAnswer i=" + i + " isFinish");
                break;
            }
